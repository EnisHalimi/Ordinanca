...
public class PacientiRepository extends EntMngClass implements PacientiInterface{ 
    ...
    @Override
    public Pacienti findByID(int id) {
        Query query=em.createQuery("SELECT p FROM Pacienti p WHERE p.nrpersonal = :nrpersonal");
        query.setParameter("nrpersonal",id);
        return (Pacienti)query.getSingleResult();
    }
